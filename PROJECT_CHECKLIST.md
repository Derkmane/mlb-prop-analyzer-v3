...
### Workstream 1 — Capture coverage — highest priority

Coverage loss is confirmed. The board workflow has one scheduled capture at 21:15 UTC. Across six committed HHR display captures, same-day slate coverage ranged from 47% to 100%; every missing game had already started when the board was pulled, with zero exceptions. The August 12 capture missed six afternoon games that started from 17:40 through 20:10 UTC. Lineup evidence also has a strong timing tradeoff: confirmed observations averaged 86 minutes before first pitch while projected observations averaged 630 minutes before first pitch.

Approved multi-capture design:

- Run an hourly schedule-aware controller. Perform a real capture only when an uncovered game is approximately 40–110 minutes from first pitch; otherwise no-op. If an uncovered game falls under 40 minutes because of a prior failure or delay, capture immediately while it remains pregame. There is no hard daily capture cap; the actual slate determines the number of captures.
- Keep the existing shared pregame gate unchanged: lifecycle status must remain scheduled and the evaluation clock must be strictly before the earlier of the BALLDONTLIE scheduled start and The Odds API commence time. At or after the cutoff, `GAME_START_REACHED` excludes the game.
- Keep same-day cumulative dedup unchanged. The existing focused regression already exercises the same prop at 12:00, 13:00, and 14:00 UTC on one date and proves timestamp-based supersession retains only the newest capture for calibration while preserving earlier snapshots as superseded evidence. A real same-day multi-capture run through grading and cumulative persistence remains the later enablement proof.
- **Snapshot-first is a correctness requirement.** On every qualifying capture run, the immutable board snapshot must be the first substantive data action before historical-shard work, enrichment, or preparation. Record run-start time, board-snapshot time, and elapsed time. A capture must fail if the board pull occurred at or after the pregame cutoff for any game that capture claims to cover.
- Trimmed HHR display storage at the accepted six-capture planning cadence is approximately 1.6–5.4 MB/day, plus the companion Batter Hits display archives. Storage is not the limiting constraint and six is not a hard capture cap.
- Future display evidence must preserve the exact lineup-source timestamp and derive lineup-evidence lead time from timestamps. Board freshness and lineup-evidence freshness are reported separately. `lineupStatus` and timing remain diagnostic only and may not alter probability, eligibility, category access, void probability, confidence, or ranking.

Mandatory implementation order — **do not reorder**:

- [ ] **Step A — Item D rate limiter.** Investigation completed 2026-08-12: `scripts/bdl-adaptive-rate-limit-utils.mjs` exists and live capture uses it; live Actions evidence for the current key reports `x-ratelimit-limit=600`, which yields a 112 ms interval at 90% utilization. Both grading paths remain on fixed 13,000 ms pacing through workflow configuration and code defaults. The current six HHR captures contain 64 archive-game incidences, implying 192 final-path BDL requests under the existing per-archive design; the pacer therefore imposes 191 intervals, approximately 41m23s at 13,000 ms versus 21.392s at 112 ms before network and processing time. The adaptive-grading correction must land in its own PR before Step B.
- [ ] **Step B — cross-archive provider-evidence reuse.** Union required game IDs across pending captures, fetch each game's final status, stats, and lineups once, and reuse that immutable provider evidence across every archive containing the game. Own PR. Do not begin before Step A is merged.
- [ ] **Step C — real worst-case grading runtime proof.** Use a real multi-capture day and literal timing evidence to prove the complete grading job remains below the existing 180-minute gate. Estimates are not acceptance evidence. Do not begin before Steps A and B are merged.
- [ ] **Step D — capture controller.** Only after Steps A–C pass, implement the approved hourly schedule-derived 40–110 minute controller, snapshot-first assertion, pregame cutoff protection, and later real same-day grading/cumulative enablement proof.

No capture-controller implementation may begin before Steps A–C complete.
...
### Version 4.5 — 2026-08-12

- Confirmed the single-fixed-time capture coverage defect from six committed HHR display captures: same-day slate coverage ranged from 47% to 100%, every missing game had already started at capture time, and August 12 lost six afternoon games.
- Recorded the approved schedule-derived multi-capture design: hourly controller, real capture only around uncovered games 40–110 minutes from first pitch, immediate under-40-minute recovery when still pregame, no hard daily cap, unchanged strict started-game exclusion, accepted display-storage footprint, and diagnostic-only lineup timing.
- Made snapshot-first a correctness requirement: the immutable board pull precedes historical shards, enrichment, and preparation; run-start/snapshot elapsed time is recorded; a capture fails if it claims a game whose board pull occurred at or after that game's pregame cutoff.
- Closed the same-day dedup design question without changing semantics: the existing 12:00/13:00/14:00 same-day regression already proves full-timestamp latest-capture supersession, superseded evidence retention, and latest-only calibration eligibility. A real multi-capture grading/cumulative proof remains required before controller enablement.
- Locked Workstream 1 implementation order as Step A Item D adaptive grading limiter, Step B cross-archive provider-evidence reuse, Step C literal real multi-capture runtime proof below the 180-minute gate, then Step D capture controller. The order may not be rearranged.
- Recorded Step A investigation evidence: the adaptive limiter already exists and live capture reports the current key at 600 requests/minute and 112 ms at 90% utilization; both grading paths still use fixed 13,000 ms pacing via workflow configuration and code defaults; the current six HHR captures imply 192 final-path BDL requests and 191 paced intervals, approximately 41m23s at 13,000 ms versus 21.392s at 112 ms before network/processing time.
- This checklist revision is documentation only. It does not change rate limiting, provider fetching, grading behavior, capture schedules, workflow execution, archive contracts, model behavior, production enablement, or ranking enablement.
