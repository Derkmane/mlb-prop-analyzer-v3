...
export function selectM9PregameEventsForCapture({
  ...
  rows.forEach((raw, index) => {
    ...
    const commenceTimeUtc = isoTimestamp(
      event.commence_time,
...
export function formatM9ArchiveFunnelReport({ funnel, status }) {
  ...
  for (const stage of snapshot.stages) {
    ...
    for (const detail of snapshot.eventDrops.filter(
      (entry) => entry.stageKey === stage.key,
    )) {
      lines.push(
        `  event drop: ${detail.eventId} | ${detail.commenceTimeUtc} | ${detail.reason}`,
      );
    }
  }
  lines.push('');
